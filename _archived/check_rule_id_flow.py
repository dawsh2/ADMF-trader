...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("rule_id_flow.log"),
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger("rule_id_flow")
...
def check_strategy_rule_id_format():
    ...
    try:
        ...
        # Get the rule_id format from the on_bar method
        on_bar_code = MACrossoverStrategy.on_bar.__code__
        on_bar_source = str(on_bar_code)
        
        # Look for correct format
        if "rule_id = f\"{self.name}_{symbol}_{direction_name}_group_{group_id}\"" in on_bar_source:
            logger.info("✅ Strategy has correct rule_id format with symbol and direction")
            return True
        elif "rule_id = f\"{self.name}_{group_id}\"" in on_bar_source:
            logger.error("❌ Strategy has incorrect rule_id format: missing symbol and direction")
            return False
        else:
            # Try to extract any rule_id format
            import re
            rule_id_pattern = re.compile(r'rule_id\s*=\s*f"([^"]*)"')
            match = rule_id_pattern.search(on_bar_source)
            
            if match:
                format_str = match.group(1)
                logger.warning(f"⚠️ Found non-standard rule_id format: f\"{format_str}\"")
                
                # Check for necessary components
                has_symbol = '{symbol}' in format_str
                has_direction = '{direction_name}' in format_str
                has_group = '{group_id}' in format_str
                
                if has_symbol and has_direction and has_group:
                    logger.info("✅ Format appears to include symbol, direction, and group_id")
                    return True
                else:
                    missing = []
                    if not has_symbol:
                    ...
                    logger.error(f"❌ Format is missing: {', '.join(missing)}")
                    return False
            else:
                logger.error("❌ Could not find rule_id format in strategy code")
                return False
    except ImportError:
...
def check_risk_manager_rule_id_processing():
    ...
    try:
        ...
        # Check for processed_rule_ids in __init__
        init_code = SimpleRiskManager.__init__.__code__
        ...
        on_signal_code = SimpleRiskManager.on_signal.__code__
        on_signal_source = str(on_signal_code)
        
        if "rule_id and rule_id in self.processed_rule_ids" in on_signal_source:
        ...
        reset_code = SimpleRiskManager.reset.__code__
        reset_source = str(reset_code)
