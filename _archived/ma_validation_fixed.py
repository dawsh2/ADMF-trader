#!/usr/bin/env python3
"""
Enhanced validation script for MA Crossover strategy.
This script implements the same strategy as the original system (5/15 SMA crossover)
with matching implementation details to validate results accurately.
"""
...
POSITION_SIZE = 19
SLIPPAGE = 0.0
CROSSOVER_THRESHOLD = 0.0  # Set to a small value to filter out noise

class SignalDeduplicator:
    """Simulate the event bus deduplication behavior."""
    
    def __init__(self):
        """Initialize the deduplicator."""
        self.processed_signals = defaultdict(set)
        self.current_position = 0  # 0 = no position, 1 = long, -1 = short
    
    def is_new_signal(self, timestamp, signal_value, symbol="MINI"):
        """
        Check if a signal is new and should be processed.
        ...
        Returns:
            bool: True if signal should be processed, False if it should be ignored
        """
        # Signal key format: "symbol_year_month_day_hour_minute_value"
        # This is similar to how rule_id might be used in the event bus
        date_part = timestamp.strftime("%Y%m%d_%H%M")
        signal_key = f"{symbol}_{date_part}_{signal_value}"
        
        # Rule 1: If we've already processed this exact signal, ignore it
        if signal_key in self.processed_signals[symbol]:
            print(f"  IGNORED: Duplicate signal {signal_key}")
            return False
        
        # Rule 2: If we already have a position in the same direction, ignore
        if self.current_position == signal_value:
            print(f"  IGNORED: Already in {signal_value} position")
            return False
        
        # Signal is valid, record it
        self.processed_signals[symbol].add(signal_key)
        
        # Update current position
        if self.current_position == 0:
            # New position
            self.current_position = signal_value
        else:
            # Closing existing and opening new position
            self.current_position = signal_value
...
def calculate_signals(df):
    ...
    # Initialize signal deduplicator to simulate event bus behavior
    ...
    # Start from index 1 to have previous values available
    for i in range(1, len(df)):
        ...
        if prev_fast <= prev_slow and curr_fast > curr_slow:
            if diff_pct >= CROSSOVER_THRESHOLD:  # Only count significant crossovers
                signal = 1
                crossover_count += 1
                
                # Check if this is a valid signal (not duplicated, etc.)
                if deduplicator.is_new_signal(timestamp, signal):
        ...
        if abs(prev_fast - prev_slow) < 0.01 or abs(curr_fast - curr_slow) < 0.01:
    ...
    print(f"\nRaw crossovers detected: {crossover_count}")
    print(f"Valid signals after deduplication: {valid_signals} ({buy_signals} buys, {sell_signals} sells)")
    
    # Save detailed signal data to CSV for inspection
    signals_df = df[df['signal'] != 0].copy()
    signals_df.to_csv('detected_signals_fixed.csv')
    print(f"Detailed signal data saved to 'detected_signals_fixed.csv'")
...
def backtest_strategy(df, use_slippage=True):
    ...
    if not trades_df.empty:
        trades_df.to_csv('backtest_trades_fixed.csv')
        print(f"\nTrade log saved to 'backtest_trades_fixed.csv'")
...
def plot_results(df, trades_df, equity_curve, title_suffix=""):
    ...
    fig.suptitle(f'MA Crossover ({FAST_WINDOW}/{SLOW_WINDOW}) Validation - Fixed Implementation {title_suffix}', fontsize=16)
    ...
    filename = f"ma_crossover_validation_fixed_{FAST_WINDOW}_{SLOW_WINDOW}_{timestamp}{title_suffix.replace(' ', '_')}.png"
...
def main():
    """Main function."""
    print("\n" + "="*60)
    print(f"MA CROSSOVER STRATEGY VALIDATION - FIXED IMPLEMENTATION ({FAST_WINDOW}/{SLOW_WINDOW})")
    print("="*60)
    print("This script uses the same implementation details as the original system:")
    print("1. Original MA calculation method (simple sum/window)")
    print("2. Less strict inequality checks (<=, >=) for crossover detection")
    print("3. Signal deduplication to match event bus behavior")
    print("4. No minimum data requirement before calculating signals")
    print("="*60)
    ...
    if metrics and metrics.get('total_trades', 0) > 0:
        ...
        print(f"The fixed validation script detected {metrics['total_trades']} trades.")
        print(f"This more closely matches the ~50+ trades seen in the original system.")
        print("Key fixes:")
        print("1. Using the same MA calculation method (direct sum/division)")
        print("2. Using <= and >= comparison instead of strict < and >")
        print("3. Adding signal deduplication similar to the event bus")
        print("4. Starting MA calculation from the first bar instead of waiting for a full window")
    else:
