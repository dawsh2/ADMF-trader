...
def calculate_signals(data, fast_window=5, slow_window=15):
    ...
    data['signal'] = 0
    data.loc[data['fast_above_slow'] & ~data['fast_above_slow'].shift(1), 'signal'] = 1  # Buy signal
    data.loc[~data['fast_above_slow'] & data['fast_above_slow'].shift(1), 'signal'] = -1  # Sell signal
...
if __name__ == "__main__":
    ...
    # First, print column names to identify the timestamp column
    print("CSV columns:", data.columns.tolist())
    
    # Now process based on the actual column names
    # Assuming there's a column that can be used as timestamp - modify as needed
    timestamp_col = None
    ...
    if timestamp_col:
        ...
        # If no timestamp column found, create an artificial index
        print("No timestamp column found. Creating artificial index.")
        data['timestamp'] = pd.date_range(start='2023-01-01', periods=len(data), freq='min')
