#!/usr/bin/env python
"""
ADMF-Trader Optimization with Fixed Position Size and Stop Loss

This script runs optimization only on the moving average periods, keeping
position size and stop loss parameters fixed throughout all tests.
"""
...
def main():
    ...
    try:
        ...
        logger.info("Starting ADMF-Trader optimization with fixed position size and stop loss...")
        ...
        for i, result in enumerate(results.get('all_results', [])):
            ...
            logger.info(f"   - Train: Score={train_score:.4f}, Return={train_return:.2f}%, Trades={train_trades}")
            logger.info(f"   - Test: Score={test_score:.4f}, Return={test_return:.2f}%, Trades={test_trades}")
        ...
        logger.info(f"  - Objective Score: {results['best_train_score']:.4f}")
        ...
        logger.info("\nTest Performance:")
        logger.info(f"  - Objective Score: {results['best_test_score']:.4f}")
