...
class EnhancedRiskManager(RiskManagerBase):
    ...
    def handle_direction_change(self, symbol, current_direction, new_direction, price, timestamp):
        ...
        # If we have a current position and new direction is different (including flat)
        ...
        # Open new position if new direction is not flat (non-zero)
        # AND the signal transition is not just to go flat (0)
        if new_direction != 0:
