    ...
    def on_signal(self, signal_event):
        ...
        try:
            ...
            if self.order_manager is not None:
                ...
                # Create order parameters with rule_id
                order_params = {
                    'symbol': symbol,
                    'order_type': 'MARKET',
                    'direction': direction,
                ...
                # Add rule_id to order parameters if available
                if rule_id:
                    order_params['rule_id'] = rule_id
                    logger.info(f"Attaching rule_id {rule_id} to order parameters")
            ...
            # Fallback approach using events if no order_manager
                ...
                # Create order event with explicit size field to avoid missing size warnings
                order_data = {
                    'symbol': symbol,
                    'order_type': 'MARKET',
                    'direction': direction,
